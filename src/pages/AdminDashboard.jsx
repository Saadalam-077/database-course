...
const AdminDashboard = ({ students, onNavigate, onLogout }) => {
  ...
  const filteredStudents = useMemo(() => {
    ...
    result.sort((a, b) => {
      switch (sortBy) {
        ...
        case 'progress':
          const aProgress = Object.values(a.progress).filter(w => w.completed).length;
          const bProgress = Object.values(b.progress).filter(w => w.completed).length;
  ...
  const stats = useMemo(() => {
    if (students.length === 0) return { total: 0, avgScore: 0, avgProgress: 0, gradeDistribution: {} };

    const totalScore = students.reduce((sum, s) => sum + (s.totalScore || 0), 0);
    const totalProgress = students.reduce((sum, s) => {
      const completed = Object.values(s.progress).filter(w => w.completed).length;
      return sum + (completed / 12) * 100;
    }, 0);

    const gradeDistribution = { 'A+': 0, 'A': 0, 'B+': 0, 'B': 0, 'C+': 0, 'C': 0, 'D+': 0, 'D': 0, 'F': 0, 'N/A': 0 };
    students.forEach(s => {
      if (gradeDistribution.hasOwnProperty(s.overallGrade)) {
        gradeDistribution[s.overallGrade]++;
      }
    });
  ...
  const StudentDetailModal = ({ student, onClose }) => {
    const completedWeeks = Object.entries(student.progress)
        ...
        <div className="bg-slate-800 rounded-2xl border border-slate-700 max-w-3xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
          ...
          <div className="p-6 grid grid-cols-3 gap-4 border-b border-slate-700">
            <div className="bg-slate-700/50 rounded-xl p-4 text-center">
              ...
              <p className={`text-3xl font-bold ${getGradeColor(student.overallGrade)}`}>{student.overallGrade}</p>
            </div>
            <div className="bg-slate-700/50 rounded-xl p-4 text-center">
              <p className="text-slate-400 text-sm mb-1">Average Score</p>
              <p className={`text-3xl font-bold ${getScoreColor(student.totalScore)}`}>{student.totalScore}%</p>
            </div>
            <div className="bg-slate-700/50 rounded-xl p-4 text-center">
      ...
      <header className="bg-slate-800/80 backdrop-blur border-b border-slate-700 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <button onClick={() => onNavigate('home')} className="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-sm transition-colors">
                ← Back to Course
              </button>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-slate-800/50 rounded-2xl p-5 border border-slate-700">
            ...
            <p className="text-3xl font-bold text-purple-400">{sections.length - 1}</p>
          </div>
        </div>
        ...
        <div className="bg-slate-800/50 rounded-2xl border border-slate-700 overflow-hidden">
          ...
          {filteredStudents.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <p className="text-slate-500 text-sm">Students will appear here once they register.</p>
            </div>
          ) : (
                  ...
                  {filteredStudents.map(student => {
                    ...
                    return (
                      <tr key={student.id} className="hover:bg-slate-700/30 transition-colors">
                        ...
                        <td className="px-4 py-3 text-center">
                          <span className={`px-3 py-1 rounded-full text-sm font-bold border ${getGradeBg(student.overallGrade)} ${getGradeColor(student.overallGrade)}`}>
